...
def showcallfeedbackfind():
    ...
    def finddata():
        ...
        if not xa :
            ...
            return
        sql="select feedbackrating ,remarks from callfeedback where callid='%s'"%(xa)
        cur.execute(sql)
        res=cur.fetchone()
        e2.delete(0,100)
        e3.delete(0,100)
    ...
    z=Label(t,text='Find in Call Feedback',font=("Segoe UI", 24, "bold"), fg="#0A3D62", bg="white", relief="raised")
    z.place(x=250,y=50)    
