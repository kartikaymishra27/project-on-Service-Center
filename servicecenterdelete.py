...
def showservicecenterdelete():
    ...
    def delete():
        ...
        if not xa:
           ...
           return
        sql="delete from servicecenter where officeid ='%s'"%(xa)
        cur.execute(sql)
        db.commit()
        db.close()
    ...
    z=Label(t,text='Delete from Service Center',font=("Segoe UI", 24, "bold"), fg="#0A3D62", bg="white", relief="raised")
    z.place(x=140,y=50)    
    
    a=Label(t,text='Office id :',font=('arial',18))
    a.place(x=100,y=150)
