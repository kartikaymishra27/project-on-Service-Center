...
def showproductfind():
    ...
    def finddata():
        ...
        if not xa:
            ...
            return
        sql="select pname,catid,warrantydays from products where productid='%s'"%(xa)
        cur.execute(sql)
        res=cur.fetchone()
        e2.delete(0,100)
        e3.delete(0,100)
    ...
    z=Label(t,text='Find in Products',font=("Segoe UI", 24, "bold"), fg="#0A3D62", bg="white", relief="raised")
    z.place(x=280,y=50)    
