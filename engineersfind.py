...
def showengineersfind():
    ...
    def finddata():
        ...
        if not xa:
            ...
            return
        sql="select ename, address ,email,phone, catid from  engineers where engid='%s'"%(xa)
        cur.execute(sql)
        res=cur.fetchone()
        e2.delete(0,100)
        e3.delete(0,100)
    ...
    z=Label(t,text='Find Engineer in Service center',font=("Segoe UI", 24, "bold"), fg="#0A3D62", bg="white", relief="raised")
    z.place(x=200,y=50)    
        
    a=Label(t,text='Engineers ID :',font=('arial',15))
    a.place(x=100,y=150)
    ...
    b=Label(t,text=' Engineers Name:',font=('arial',15))
    b.place(x=100,y=270)
