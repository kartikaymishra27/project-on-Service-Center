...
def showcustomerfind():
    ...
    def finddata():
        ...
        if not xa:
            ...
            return
        sql="select cname, address ,email,phone, catid,productid from  customer where cusid='%s'"%(xa)
        cur.execute(sql)
        res=cur.fetchone()
        e2.delete(0,100)
        e3.delete(0,100)
    ...
    z=Label(t,text='Find customer in Service center',font=("Segoe UI", 24, "bold"), fg="#0A3D62", bg="white", relief="raised")
    z.place(x=200,y=50)    
