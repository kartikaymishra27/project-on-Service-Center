...
def showcallfeedbackdelete():
    ...
    def delete():
        ...
        if not xa :
           ...
           return
        sql="delete from callfeedback where callid='%s'  "%(xa)
        cur.execute(sql)
        db.commit()
        db.close()
    ...
    def filldata():
         db = pymysql.connect(host='localhost',user='root',password='root',database='servicecenter')
         cur = db.cursor()
         sql="select callid  from callfeedback "
         lt=[]
         cur.execute(sql)
         data=cur.fetchall()
         for res in data:
    ...
    z=Label(t,text='Delete from Call Feedback',font=("Segoe UI", 24, "bold"), fg="#0A3D62", bg="white", relief="raised")
    z.place(x=140,y=50)    
    
    a=Label(t,text='Call id :',font=('arial',18))
    a.place(x=100,y=150)
