...
def showproductdelete():
    t=tkinter.Tk()
    t.geometry('600x400')
    t.title('Products Delete')
    def cx():
        t.destroy()
    ...
    def delete():
        ...
        if not xa:
           ...
           return
        sql="delete from products where productid='%s'"%(xa)
        cur.execute(sql)
        db.commit()
        db.close()
    ...
    z=Label(t,text='Delete from Products ', font=("Segoe UI", 24, "bold"), fg="#0A3D62", bg="white", relief="raised")
    z.place(x=140,y=50)    
    
    a=Label(t,text='Product id :',font=('arial',20))
    a.place(x=100,y=150)
