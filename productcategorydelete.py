...
def showproductcategorydelete():
    t=tkinter.Tk()
    t.geometry('600x400')
    t.title("product category delete")
    def cx():
        t.destroy()
    ...
    def delete():
        ...
        if not xa :
           ...
           return       
        sql="delete from productcategory where catid='%s' "%(xa) 
        cur.execute(sql)
        db.commit()
        db.close()
    ...
    z=Label(t,text='Delete from Product category', font=("Segoe UI", 24, "bold"), fg="#0A3D62", bg="white"
            , relief="raised")
    z.place(x=140,y=50)    
    
    a=Label(t,text='Category ID:',font=('arial',18))
    a.place(x=100,y=150)
